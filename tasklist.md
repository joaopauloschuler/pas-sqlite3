...
- [~] **9.4.divbug.89** Empty driver diagnostic (carved from `9.4.4.g-unbucketed` 2026-05-16) — **12 pas-soft tests** (`corruptB`, `e_changes`, `e_totalchanges`, `fuzz`, `index4`, `index5`, `join6`, `joinA`, `joinB`, `joinD`, `manydb`, `tkt3080`) FAIL but `bin/tcl-failure-logs/<base>.{err,out}` capture no diagnostic — driver swallows the message or the tests abort outside `tcltest`.  Action: instrument `TclTestDriver` to dump the last N lines of stdout/stderr on any non-PASS exit so these become triageable.  Instrumentation landed (TclTestDriver.pas: `WriteFailLogs` now emits a header — test path, spawn cmd, exit-code, byte counts — and appends a 50-line tail block; empty streams write `(empty)`).  Smoke: join6 now reveals `exit-code: 134` + `double free or corruption (!prev)`; e_changes/manydb reveal `exit-code: 139` (SIGSEGV) past the last `Ok` line.  Per-test root-causing of .001..012 remains TODO.
  ...
  - [ ] **9.4.divbug.89.008** `joinA` — SIGABRT (exit-code 134) `free(): invalid next size (normal)` during `joinA-1.100` (joinA.test:67..75) — 3-way INNER JOIN USING(c,d)/(a,b,f)/(a,c,d,f) with `ORDER BY 1 nulls first, 3 nulls first`.  Heap corruption in USING-column resolution or NULLS FIRST sorter (where.c USING expansion).  Re-bucket as SIGABRT-using-join-heap.
  - [ ] **9.4.divbug.89.009** `joinB` — SIGABRT (exit-code 134) `double free or corruption (!prev)` during `joinB-2` (joinB.test:61..72) — 5-way INNER JOIN USING(a,b) chain with `WHERE a<>13 ORDER BY 1 NULLS FIRST`.  Heap corruption analogous to .008 (multi-key USING + NULLS FIRST sort).  Re-bucket as SIGABRT-using-join-heap.
  - [ ] **9.4.divbug.89.010** `joinD` — SIGABRT (exit-code 134) `free(): invalid next size (normal)` during `joinD-8` (joinD.test:140..148) — 4-way INNER JOIN with `ORDER BY coalesce(...)`; joinD-1..7 PASS.  Heap corruption in coalesce-over-join-output expression list, likely same family as .008/.009.  Re-bucket as SIGABRT-join-heap.
