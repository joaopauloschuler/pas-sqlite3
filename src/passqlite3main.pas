...
begin
  ...
  if (pColl <> nil) and (pColl^.xCmp <> nil) then begin
    ...
    { If collation sequence pColl was created directly by a call to
      ...
      to be called. }
    if (pColl^.enc and (not SQLITE_UTF16_ALIGNED)) = enc2 then begin
      aColl := PTCollSeq(sqlite3HashFind(@db^.aCollSeq, zName));
      for j := 0 to 2 do begin
        pJ := PTCollSeq(PAnsiChar(aColl) + j * SizeOf(TCollSeq));
        if pJ^.enc = pColl^.enc then begin
