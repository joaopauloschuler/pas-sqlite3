...
- [X] **9.4.divbug.24.b.4** aggnested-3.11 "misuse of aggregate: max()" — FindNestedAggToOuter gate now also matches TK_AGG_FUNCTION (MarkOrRejectAggInWhere had pre-promoted max() with op2=0 before the outer-rebind pass ran), codegen.pas:11284; C resolve.c:1337..1352.
...
- [X] **9.4.divbug.62** test1.c `sqlite3_*` Tcl trampolines — ALL ported (62.a–f: mprintf/stmt-introspect/bind/status+limit/function-mgmt/config+legacy-key). BRIDGE DONE; live-swept a4 2026-06-04, zero missing-cmd aborts. Residual 17 pas-soft (pcache2/pendingrace/…) are engine/config-shape bugs, not bridge (mem/pcache `sqlite3_config` variadic shapes → class-C, see divbug.90 backlog).
...
- [X] **9.4.divbug.63** tcl-shim helper cmds — ALL ported (run_thread_tests/test_cli_invocation/test_find_cli/test_find_sqldiff/tcl_variable_type/breakpoint/database_may_be_corrupt/explain_no_trace/file_control_reservebytes/faultsim_test_result); a4 2026-06-04 added the live-found stragglers sorter_test_fakeheap + sorter_test_sort4_helper (test1.c:8469/8503 → TestModuleTest1.pas) and do_test_with_ansi_output (tester.tcl:812 → tester_min.tcl). BRIDGE DONE. Residual non-bridge: quota-glob=test_quota VFS (class-C), sort4/select9=sorter perf, shell1=needs real CLI binary. Cites 7be23b7,1800d46,8de8be5.
- [X] **9.4.divbug.63.sweep** Full live-driver bridge sweep (a4 2026-06-04) — swept all 230 unswept/pas-soft files: ZERO remaining missing-command aborts. Ported the last 3 real gaps found corpus-wide: `sqlite3BitvecBuiltinTest` (bitvec.c:394 → passqlite3util.pas + TESTCTRL_BITVEC_TEST in passqlite3main.pas + test2.c:671 trampoline) → **bitvec.test now PASS 0 err**; `btree_ismemdb` (test3.c:553) and `tcl_objproc` (test1.c:7831) → TestModuleTest1.pas (pragma/tclsqlite bridges closed, residuals engine/harness). Regression 108/4 == baseline. Class-C backlog (whole VFS/module ports, NOT bridge trampolines): crash VFS (test6.c), quota VFS (test_quota.c), intarray module (test_intarray.c), fts3 test module make_fts3record (fts3_test.c), mem/pcache `sqlite3_config` variadic shapes (test_malloc.c), `sqlite_orig` rename mechanic (tclsqlite.c).
...
- [X] **9.4.divbug.76** View-column resolution / no-FROM-inside-FROM-subquery SIGSEGV — VERIFIED ALREADY FIXED 2026-05-23 (stale note). tkt3346 5/5 PASS; repro `SELECT (SELECT y FROM (SELECT 1 AS y)) FROM t1` returns 1 (no crash), matches oracle; parity 1026/1026. Resolved transitively by the FROM-subquery materialisation / compound-flatten work (see memory `from_subquery_materialized_not_coroutine`, `flatten_runs_late_redispatch_compound`).
...
- [ ] **9.4.divbug.87** Result divergence cluster (carved from `9.4.4.g-unbucketed` 2026-05-16) — **73 pas-soft tests** emit `got:` lines that do not match the C oracle.  Subdivided into `result-divergence` (68: e.g. `backup5`, `badutf`, `capi2`, `colmeta`, …) and `malformed-corrupt-vector` (5: `backup4`, `corruptM`, `in2`, `rowhash`, …).  Each test needs an individual bisect; this single bullet placeholds until root-cause splits emerge.  Full list classified in `/tmp/unbk_sig2.tsv` from the 2026-05-16 sweep.
  ...
  - [X] **9.4.divbug.87.023** `in6` — PASS 10/10 after the OBLopt early-out fix (commit 941597f, see .046). The WO_IN equality-chain arm was already ported (codegen.pas:19210); the prior "missing WO_IN arm" diagnosis was stale.
  ...
  - [X] **9.4.divbug.87.027** `index8` — index8.test + bestindex8.test PASS (verified 2026-05-28).
  ...
  - [X] **9.4.divbug.87.030** `indexedby` — indexedby.test PASS 70/70 (verified 2026-05-28).
  ...
  - [X] **9.4.divbug.87.041** `join7` — join7.test now FULL PASS (0 errors) as of commit 617f66c (2026-05-24). Final residual join7-*.70 (`SELECT t1.* FROM t2 LEFT JOIN (dual JOIN t1 ON true) ON b=c` → `no such table: t1`) fixed by porting expandStar's SF_NestedFrom `T.*` arm (select.c:6168..6230): a nested-from wrapper is matched by reverse-mapping each pre-bound TK_COLUMN result column to its originating inner SrcItem (new helper NestedFromColMatchesTable, mirrors FindWrapperEListIdx) instead of being skipped on name-mismatch. ExplainParity 1026/1026. (Earlier .20/.30/etc. fixes per original note.)
  ...
  - [X] **9.4.divbug.87.046** `limit2` — FIXED 2026-05-23 (commit 941597f), limit2 24/24. Triage premise (missing WO_IN arm) was stale — the WO_IN equality-chain arm was already ported (codegen.pas:19210). Real root cause: the IN+ORDER BY+LIMIT inner scan never terminated early at runtime. `sqlite3WhereOrderByLimitOptLabel` was a stub returning iBreak; ported where.c:101..111 (return iContinue when !bOrderedInnerLoop else inner level addrNxt), and routed the Top-N sorter push OP_IdxLE through labelOBLopt (select.c:867..869, captured after WhereBegin per select.c:8295). ExplainParity 1026/1026.
  ...
  - [X] **9.4.divbug.87.055** `misc5` — engine bug fixed: SRT_EphemTab/SRT_Table now admitted as obCandidate in GROUP-BY+DISTINCT arm + sorter-drain disposal (codegen.pas:34442, 35201); inner FROM-subquery `SELECT DISTINCT <cols>,<agg> GROUP BY <other> ORDER BY` now materialises rows ([] → [one]). misc5-3.1 test still nominally FAILs (asserts `{two}` but test comments mark result indeterminate — 'one'/'two'/'three' all valid). Parity 1026/1026, select1 no new regression.
  ...
  - [X] **9.4.divbug.87.061** `orderbyA` — VERIFIED ALREADY FIXED 2026-05-23 (stale note), orderbyA 98/98 PASS. The full WHERE_SORTBYGROUP / sqlite3WhereIsOrdered routing is present: WHERE_SORTBYGROUP passed to WhereBegin (codegen.pas ~33340), groupBySortLoc = (WhereIsOrdered=nGroupBy) (~33369), "USE TEMP B-TREE FOR GROUP BY" gated on groupBySortLoc<>0 (~33407), "USE TEMP B-TREE FOR ORDER BY" in the needSortOB drain (~33718), orderByGrp/pOrderBy cancellation (~33381). The literal C groupBySort=0 direct-read bytecode arm is not ported (the port keeps the sorter but suppresses the banner) — verified EQP-equivalent vs oracle and parity 1026/1026.
  ...
  - [X] **9.4.divbug.87.064** `rowhash` — rowhash.test PASS 10/10 (verified 2026-05-28).
...
- [ ] **9.4.divbug.88** Tcl-bridge command/subcommand long-tail (carved from `9.4.4.g-unbucketed` 2026-05-16) — **62 pas-soft tests** still hit bridge gaps after `9.4.divbug.62/.63/.64/.65` closed the high-frequency surface.  Subdivided into `invalid/unknown command` (55: `badutf2`, `bindxfer`, `cacheflush`, `capi3d`, …) and `unknown subcommand "null"` / `db <other-subcmd>` (7: `indexexpr1`, `joinH`, `join`, `json102`, …).  Port the remaining `db ?subcommand?` and top-level Tcl-bridge entry points until the cluster drains.  **BRIDGE SURFACE FULLY DRAINED a4 2026-06-04** — all 60+ children [X]; sole remaining open child is `88.068.b` (multi-overflow-page reopen SIGSEGV = ENGINE, not bridge). Parent stays `[ ]` only to track that engine carve-out.
  ...
  - [X] **9.4.divbug.88.020** `corruptI` — PASS 23/23; corruptI-6.1 fixed by forcing u32 wrap in clearCellOverflow nOvfl (btree.c:6987) + btreeParseCellAdjustSizeForOverflow surplus (btree.c:1198); FPC was widening to i64 / signed-mod producing nLocal=65535 + nOvfl=8.45M instead of the gcc u32-overflow path that yields nOvfl=0 → OK.
  - [X] **9.4.divbug.88.021** `corruptJ` — PASS 5 / 317 (cured by .014).
  - [X] **9.4.divbug.88.022** `corruptK` — BRIDGE DONE (sqlite_dbpage ported as src/passqlite3dbpage.pas, 3.2 passes); lone residual corruptK-3.3 is an error-text/engine divergence, not bridge. 1 fail / 9. Lone residual: corruptK-3.3 expects `database disk image is malformed`, Pas returns `malformed database schema () - orphan index`. Same callback path (prepare.c:176 / passqlite3main.pas:3333), but Pas takes the orphan-index branch (argv[4]=empty) while C takes the "cr..." CREATE branch and bubbles up SQLITE_CORRUPT with the generic msg. Divergence is in sqlite_schema cell decode for the bizarre corrupted page-1 blob from 3.2 (CREATE TABLE leaf cell at offset 0x3a0, payload len 0x5e). Deep btree-cell-decode parity issue; not a 45-min fix.
  ...
  - [X] **9.4.divbug.88.033** `e_walauto` — BRIDGE DONE (nonzero_reserved_bytes); residual = WAL `-shm` OS layer, not bridge. re-triaged 2026-05-17: `nonzero_reserved_bytes` SOURCE-ERROR cleared by 88.012 port (PasTclSqlite.pas:4881). e_walauto.test now runs e_walauto-1.1.0/1.1.1 (2 PASS / 0 errors) then SOURCE-ERROR: `couldn't open "test.db-shm": no such file or directory` — WAL `-shm` not created (no real WAL-mode mmap shm in current OS layer). Residual is OS/WAL-shm, separate bucket.
  ...
  - [X] **9.4.divbug.88.036** `fallocate` — fallocate.test PASS 18/18 (verified 2026-05-28).
...
- [ ] **9.4.divbug.90** Extension / SQL function / VFS registration residue (sibling of `9.4.divbug.66`, carved from `9.4.4.g-unbucketed` 2026-05-16) — **8 pas-soft tests**: 6 `no such extension` (`btree02`, `extension01`, `func4`, `indexexpr2`, …), 1 `no such function` (`func9`), 1 `no such vfs: devsym` (`io`).  Each pin in the C build is a known extension/function/VFS shim; port or auto-register at db-open following the `.66` template.  Progress 2026-05-17: 7/8 registration shims landed (5 aExtension[] rows in TestModuleTest1.pas — eval/fileio/totype/explain/wholenumber, all Pas ports already existed; unistr_quote builtin in passqlite3codegen.pas via quoteFunc + pUserData=1, mirrors func.c:3340).  3/8 now PASS via driver (`extension01`, `func4`, `func9`); 4/8 reach the engine post-shim and re-bucket as non-registration engine bugs (`btree02`, `indexexpr2`, `memdb`, `misc8`); 1/8 (`io`) still wants the full `test_devsym.c` VFS port — out of scope for the registration drain.
  ...
  - [X] **9.4.divbug.90.005** `indexexpr2` PASS 126/126 (verified 2026-05-28, stale entry — buckets (b)/(c) already cleared by prior session fixes; TestExplainParity 1026/1026).
...
- [ ] **9.4.divbug.91** Tcl harness helper gaps (carved from `9.4.4.g-unbucketed` 2026-05-16) — **16 pas-soft tests** on missing test-harness plumbing (engine behaviour not exercised): `md5sum` Tcl command (5: `backup_ioerr`, `backup`, `fuzz3`, `interrupt`, `trans2`); arbitrary missing tclvars (4: `join3`, `savepoint2`, `tkt3992`, `types`); `cmdlinearg(soft-heap-limit)` array (2: `avtrans`, `capi3b`); `SQLITE_MAX_VARIABLE_NUMBER` tcl-const (1: `bind`); `QRF not available in this build` build-flag gap (2: `qrf01`, `qrf02`); `no files matched glob "*malloc*.test"` (1: `mallocAll`); `couldn't read file "-"` stdin input (1: `memleak`).  Progress 2026-05-17: md5sum SQL aggregate now auto-registered on every connection (PasTclSqlite.pas DbMain — calls Md5_Register after sqlite3_open_v2, mirrors test_func.c:723..726 autoinstall_test_functions / auto_extension), and tester_min.tcl seeds `bitmask_size=64` (test1.c:9335..9438), `SQLITE_MAX_VARIABLE_NUMBER=32766` (test_config.c:817), `cmdlinearg(soft-heap-limit)=0` (tester.tcl:378), `sqlite_options(utf16)=1` (test_config.c:705).  3/16 closed; 10/16 now reach the engine (residuals are not harness gaps and re-bucket below); 3/16 remain genuine harness gaps (backup family, qrf, mallocAll/memleak).
  - [X] a4 2026-06-05: strftime/sqlite3_libversion_number/save_prng_state/reset_prng_state ported (TestModuleTest1.pas, 3806962) → date4 PASS, fts3corrupt4 76→158, tclsqlite runs to completion. scanstatus/scanstatus2 PASS via `sqlite_options(scanstatus) 0` pin (8d956cb, oracle lacks SCANSTATUS). QRF: NOT a port target — oracle also lacks qrf.h/qrf.c (would diverge); fix is a skip-guard like preupdate.test 6.40.3.
  - [X] **9.4.divbug.92** incrvacuum `no such column: t2.oid` — qualified outer rowid-alias (X.oid/.rowid/._rowid_) in correlated subq unbound; rowid arm added to resolveOuterDotInExpr (codegen.pas, resolve.c:471/636/637), 052474b. ExplainParity 1026/1026; residual incrvacuum page-count/malformed fails pre-existing+deferred.
