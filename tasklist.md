...
- [ ] **9.4.divbug.71.c** ALTER TABLE ADD COLUMN error wrapper: STRICT t4 reports `malformed database schema (t4) - unknown datatype for t4.d: "VARCHAR"` instead of C's `error in table t4 after add column: unknown datatype for t4.d: "VARCHAR"` (strict1-7.2/7.3).  Likely cite: ../sqlite3/src/alter.c ADD COLUMN reload-schema error path.
