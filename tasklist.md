...
- [ ] **9.4.divbug.24.b** aggnested-3.3 wrong scalar-subquery value + aggnested-3.11 SIGSEGV — `(SELECT … FROM t2)` inside an aggregate outer that references the outer's value1 / `max(value1)` folds against the wrong row (3.3) or segfaults (3.11 — VDBE dispatch jumps to a bad function pointer when the inner WHERE references an outer aggregate function).  Sub-fix `0c…` landed: 3.2 is now correct after gating flattenSubquery on `pSub->pGroupBy/pHaving/SF_HasAgg == 0` at codegen.pas:29744..29754 (mirrors C select.c:7796..7797 assertion).  Residual 3.3/3.11 needs the aggregate-context substitution + scalar-subquery codegen arm — deeper, deferred.
