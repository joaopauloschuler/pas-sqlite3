...
- [ ] **9.4.divbug.71.b** PRAGMA quick_check/integrity_check leaks btree-level diagnostics ("*** in database main ***\n2nd reference to page 2\nPage N: never used") on writable_schema-remapped tables instead of just the STRICT type messages.  Test: strict2 (all 1.x subtests).  Likely needs porting the wrapping in ../sqlite3/src/pragma.c integrity_check arm so STRICT errors aren't co-mingled with btree page checks, or fixing the page-reuse accounting when rootpage is reassigned.
...
- [ ] **9.4.divbug.87** Result divergence cluster (carved from `9.4.4.g-unbucketed` 2026-05-16) — **73 pas-soft tests** emit `got:` lines that do not match the C oracle.  Subdivided into `result-divergence` (68: e.g. `backup5`, `badutf`, `capi2`, `colmeta`, …) and `malformed-corrupt-vector` (5: `backup4`, `corruptM`, `in2`, `rowhash`, …).  Each test needs an individual bisect; this single bullet placeholds until root-cause splits emerge.  Full list classified in `/tmp/unbk_sig2.tsv` from the 2026-05-16 sweep.
  ...
  - [ ] **9.4.divbug.87.066** `securedel` — ! securedel-1.0 got: [0].  Partial: ported PragTyp_SECURE_DELETE read+write arm (codegen.pas, pragma.c:624..643) — previously the constant-default emitter returned 0 and silently no-op'd writes, so `PRAGMA secure_delete=ON` never toggled `BTS_SECURE_DELETE` and read-back was always 0.  Now propagates to every attached pBt when no schema prefix (matches `pId2->n==0` arm) and reuses already-ported `sqlite3BtreeSecureDelete` (btree.pas:8024).  securedel 13/15 PASS (1.3-1.8 + 2.x all flip ok); residual 1.0/1.1/1.2 fail because tester_min.tcl:244 `ifcapable` stub is unconditional-true → `ifcapable fast_secure_delete` returns true and DEFAULT_SECDEL is set to 2, but this build truly defines neither SQLITE_SECURE_DELETE nor SQLITE_FAST_SECURE_DELETE (btree.c:2695..2699) so the port honestly returns 0.  Harness-stub artifact, not a port bug — proper fix is wiring `ifcapable` to consult `sqlite_options()` for `fast_secure_delete`/`secure_delete` (tester_min.tcl:1179), out of scope.  Deferred for 1.0/1.1/1.2.
