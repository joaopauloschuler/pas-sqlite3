...
- [ ] **10.1f** Out-of-scope dependencies (session, archive, recover) may stub with the upstream `SQLITE_OMIT_*` "feature not compiled in" message. Gate: `tests/cli/10f_misc/`.
  ...
  - [X] **10.1f.14** `.vfsinfo` — gated by `src/tests/TestShellMisc.pas` (vfsinfo-nosuch/vfsinfo-bogus, 2 subtests; both silence-path on a `:memory:` db, matching upstream byte-for-byte). Side-fix: cmdVfsinfo argv gate tightened from `nArg >= 1` to `nArg = 1` to match C's `nArg==2 ? azArg[1] : "main"` (shell.c.in:11999). Success-path byte-parity is blocked by cross-cutting pre-existing port-state divergences (unix VFS iVersion=2 vs 3, missing unix-excl/dotfile/none locking-style shims, `:memory:` open routing through unix rather than memdb) — call out as not in scope for 10.1f.14.
  - [X] **10.1f.15** `.vfslist` — handler audited against shell.c.in:12012..12028; format strings and pNext-walk are byte-identical (passqlite3shell.pas:6564..6585). Side-fix: removed unconditional `sqlite3VfsstatInit(p^.db)` from openDb so the "vfslog"-named vfsstat wrapper VFS no longer appears in the chain (passqlite3shell.pas:1210..1218; mirrors the 10.1.90 cksumvfs precedent of "shim exported but not auto-installed"). Stdout byte-parity gating documented in TestShellMisc as deferred until the unix VFS iVersion-3 bump and the unix-excl/dotfile/none shim registrations land — both cross-cutting.
  - [X] **10.1f.16** `.vfsname` — gated by `src/tests/TestShellMisc.pas` (vfsname-bare/vfsname-main/vfsname-nosuch/vfsname-multi, 4 subtests; every reachable invocation on `:memory:` emits zero output since memdb's xFileControl doesn't implement VFSNAME, matching upstream byte-for-byte). Side-fix: cmdVfsname argv gate tightened from `nArg >= 1` to `nArg = 1` to match C's `nArg==2 ? azArg[1] : "main"` (shell.c.in:12031), so multi-token forms collapse to "main" as upstream does.
