...
- [ ] **6.40.1** **FTS3/FTS4 full-text search** (ENGINE/EXT, XL — not yet ported at all; ~21 kLOC across `../sqlite3/ext/fts3/`). `no such module: fts4` (19 tests), `fts4aux` (26), `fts3tokenize` (2), `no such function: fts3_tokenizer_test` (77). End state: auto-register the `fts3`/`fts4`/`fts4aux`/`fts3tokenize` modules + the `fts3_tokenizer`/`fts3_tokenizer_test` SQL funcs from `sqlite3Fts3Init` (fts3.c:4102), called at openDatabase time. Sub-tasks below are **dependency-ordered**; new home unit `src/passqlite3fts3.pas` (built by build.sh + build_tcl_lib.sh).
  - **Cluster A — tokenizer subsystem (the easy wins; entirely independent of the 12 kLOC segment/vtab core).** Each item drives the `sqlite3_tokenizer_module` vtable directly, so each is unit-testable in isolation against the C oracle via a small `DiagFts3Tok` probe — no FTS vtab needed. Landing all of Cluster A flips the two biggest buckets: `fts3_tokenizer_test` (77) and `fts3tokenize` (2).
    ...
    - [ ] **6.40.1.c** `fts3_tokenizer1.c` (234L) — the built-in `simple` tokenizer module (`sqlite3Fts3SimpleTokenizerModule`). Smallest tokenizer; verify token boundaries/offsets vs oracle. EASY.
    - [ ] **6.40.1.d** `fts3_porter.c` (662L) — the `porter` stemmer tokenizer (`sqlite3Fts3PorterTokenizerModule`). Self-contained Porter algorithm; verify stems for a word list vs oracle. EASY-MED.
    - [ ] **6.40.1.e** `fts3_unicode2.c` (383L) — Unicode codepoint category tables + `sqlite3FtsUnicodeIsalnum`/`Fold`. Pure data/logic, no I/O. Gated by `!SQLITE_DISABLE_FTS3_UNICODE` (oracle enables it). EASY.
    - [ ] **6.40.1.f** `fts3_unicode.c` (397L) — the `unicode61` tokenizer (`sqlite3Fts3UnicodeTokenizer`); depends on 6.40.1.e. MED.
    - [ ] **6.40.1.g** `fts3_tokenizer.c` (516L) — the tokenizer registry: `sqlite3Fts3InitHashTable` (installs the `fts3_tokenizer` SQL func eponymous-table) + `sqlite3Fts3InitTokenizer` + `registerTokenizer`. **Includes the `SQLITE_TEST`-gated `fts3_tokenizer_test` SQL func (fts3_tokenizer.c:227/482..508) — this is what 77 tests need.** Depends on 6.40.1.b. MED. ← biggest single test win.
    - [ ] **6.40.1.h** `fts3_tokenize_vtab.c` (459L) — the `fts3tokenize` virtual table module (`sqlite3Fts3InitTok`). Confirmed to depend ONLY on the tokenizer hash + a registered tokenizer (no SegReader / no `fts3_write` core), so it lands as soon as Cluster A is up. Flips `fts3tokenize` (2). MED.
  - **Cluster B — the FTS index core (XL; this is the hard 80%).** Needed before any `fts3`/`fts4` data table or `fts4aux` works.
    - [ ] **6.40.1.i** `fts3_expr.c` (1316L) — the `MATCH` query-expression parser (`sqlite3Fts3ExprParse`) + the `SQLITE_TEST` `fts3_exprtest`/`fts3_exprtest_rebalance` interface (fts3_expr.c:1203..1311). Depends on the tokenizer infra (Cluster A). MED-L.
    - [ ] **6.40.1.j** `fts3_write.c` (5856L) — segment b-tree writer / `Fts3MultiSegReader` / doclist merge / pending-terms hash / `optimize`/`merge`/`integrity-check`. The engine's core; **split internally when started** (segdir/SegReader read path; pending-terms + flush write path; doclist varint codecs; deferred-token + NEAR; incremental-merge). Includes the **FTS4-only `%_docsize` + `%_stat` shadow tables** (`bHasDocsize`/`bHasStat`, fts3.c:679..735 drive the writes here). XL.
    - [ ] **6.40.1.k** `fts3.c` (6203L) — the `fts3`/`fts4` vtab module (xCreate/xConnect/xBestIndex/xFilter/xColumn/xUpdate, content/contentless/external-content options, `fts3Module`). Depends on 6.40.1.i + 6.40.1.j. **Split internally** (schema parse + DDL; xBestIndex/full-scan vs MATCH; cursor/xFilter; xColumn + auxiliary `optimize`). XL. Flips `no such module: fts4` (19) once the module name is registered (.o), but the FTS4 *feature* tests depend on the FTS4-specific arms below actually landing. **FTS4 ≠ a separate port — same `fts3Module` registered twice (fts3.c:4176/4181); the only fork is `isFts4=(argv[0][3]=='4')` (fts3.c:1187).** FTS4-specific surface to track here:
      - [ ] **6.40.1.k.1** key=value option parser — `content=`, `compress=`/`uncompress=`, `order=`, `prefix=`, `matchinfo=`, `languageid=`, `notindexed=` (fts3.c:775 `fts3IsSpecialColumn`, gated `isFts4` at fts3.c:1252..1253).
      - [ ] **6.40.1.k.2** `%_docsize`/`%_stat` DDL + the `bHasStat==2` lazy-detect for legacy tables (fts3.c:728..735, 3562..3573); shadow-table *writes* live in 6.40.1.j.
      - [ ] **6.40.1.k.3** verify FTS4 feature tests (contentless/external-content, prefix indexes, languageid, notindexed) once .k.1/.k.2 + core land.
    - [ ] **6.40.1.l** `fts3_snippet.c` (1778L) — `snippet()`/`offsets()`/`matchinfo()` auxiliary functions (overloaded in `sqlite3Fts3Init`). Depends on 6.40.1.k; the richer `matchinfo()` format codes only resolve correctly once the FTS4 `%_stat` table (6.40.1.k.2/.j) is present. L.
