...
- [ ] **9.4.divbug.88** Tcl-bridge command/subcommand long-tail (carved from `9.4.4.g-unbucketed` 2026-05-16) — **62 pas-soft tests** still hit bridge gaps after `9.4.divbug.62/.63/.64/.65` closed the high-frequency surface.  Subdivided into `invalid/unknown command` (55: `badutf2`, `bindxfer`, `cacheflush`, `capi3d`, …) and `unknown subcommand "null"` / `db <other-subcmd>` (7: `indexexpr1`, `joinH`, `join`, `json102`, …).  Port the remaining `db ?subcommand?` and top-level Tcl-bridge entry points until the cluster drains.
  ...
  - [ ] **9.4.divbug.88.018** `corruptG` — cured-but-residual: now 5 fail / 150 (was 0/20) after .014 freelist_count fix; remaining 5 are deeper btree-malform arms.
  - [X] **9.4.divbug.88.019** `corruptH` — PASS 9 / 1186 (cured by .014).
  - [ ] **9.4.divbug.88.020** `corruptI` — cured-but-residual: 23 fail / 1919 (was 0/17) after .014; remaining deeper.
  - [X] **9.4.divbug.88.021** `corruptJ` — PASS 5 / 317 (cured by .014).
  - [ ] **9.4.divbug.88.022** `corruptK` — cured-but-residual: 9 fail / 883 (was 0/47) after .014; remaining deeper.
  ...
  - [~] **9.4.divbug.88.068** `cksumvfs` full port — full 1:1 Pascal port of `../sqlite3/ext/misc/cksumvfs.c` (820 C lines → 751 line `src/passqlite3cksumvfs.pas`, already existed pre-task in mostly-complete form) is now wired to the Tcl trampolines (`TestModuleTest1.pas:2329..2354` now call the real `sqlite3_register_cksumvfs` / `_unregister_cksumvfs` instead of returning SQLITE_OK stubs).  Auto-extension `cksmRegisterFunc` ported (cksumvfs.c:767..783) so `verify_checksum()` registers on every new connection.  Smoke confirms `sqlite3_vfs_find('cksmvfs')` returns the shim and `verify_checksum(blob)` evaluates.  cksumvfs.test still SIGSEGVs at 1.3 (3 sub-asserts pass, was 3 before) — independent engine bugs broken out into sub-buckets .068.a and .068.b below.
    - [ ] **9.4.divbug.88.068.a** `PRAGMA <unknown>` must dispatch to `SQLITE_FCNTL_PRAGMA` — Pascal codegen doesn't fall back to `sqlite3FileControl(SQLITE_FCNTL_PRAGMA)` for unknown pragmas, so cksmFileControl never sees `PRAGMA checksum_verification`.  C ref: `pragma.c:184..213` (the `if( zRight ){ ... }` end-of-pragma FCNTL fallback emits `OP_Pragma`/`OP_VFSCall`).  Required to drive cksumvfs.test past sub-test 1.2.
    - [ ] **9.4.divbug.88.068.b** Multi-overflow-page INSERT after `db close; sqlite3 db file.db` reopen SIGSEGVs — reproduced via tester_min on a fresh test.db **with or without** cksumvfs in the VFS chain, so the bug is in core pager/btree overflow-page reload, not in the shim.  Repro: `page_size=4096; INSERT INTO t VALUES(randomblob(5000), randomblob(1500)); db close; sqlite3 db file.db; SELECT * FROM t;` → SEGV.  Likely overflow-page chain pointer reload after detach/reattach.  Cite: `btree.c` accessPayload + freeOverflowPage paths.
  - [~] **9.4.divbug.88.069** `multiplex` full port — landed `src/passqlite3multiplex.pas` (917 Pascal lines from the ~1370-line `../sqlite3/src/test_multiplex.c`).  Real chunk-file shim VFS: multiplexOpen / multiplexRead / multiplexWrite split I/O across `foo.db`, `foo.db001`, … chunks; multiplexDelete sweeps journal/wal chunks; multiplexFileControl exposes MULTIPLEX_CTRL_* and the `multiplex_truncate / _enabled / _chunksize / _filecount` pragmas; multiplexFuncInit auto-extension registers the `multiplex_control()` SQL fn.  Tcl trampolines `sqlite3_multiplex_initialize / _shutdown / _control` rewired in TestModuleTest1.pas (88.025 / 88.055 STUBs replaced).  Counts: `delete_db` 4→18 sub-tests reached (4× deeper; remaining 14 fail on the unrelated `sqlite3_delete_database` Tcl command and the `SQLITE_ENABLE_8_3_NAMES` URI gate, both out of scope); `multiplex4` 4 pass + 10 fail (most failures are the 8_3_NAMES `.db001` vs `.001` rename and pragma `multiplex_truncate` echo).  Regression gate: 100/101 (TestFuzzDiff red as expected, 5264/5264 assertions).  Deferred sub-buckets:
    - [ ] **9.4.divbug.88.069.a** `SQLITE_ENABLE_8_3_NAMES` arms in `multiplexFilename` (extension truncation) + `multiplexSubOpen` (journal/wal offset rename, chunk overflow → `SQLITE_FULL`).  Drives the remaining ~10 `multiplex4` failures and the `delete_db` 2.x sub-tests.  C ref: `test_multiplex.c` 8_3_NAMES gated blocks.
    - [ ] **9.4.divbug.88.069.b** `sqlite3_delete_database` Tcl command — used by `delete_db.test`; needs Tcl trampoline for the public extension API (`ext/misc/delete.c` or equivalent), independent of the multiplex VFS itself.
...
- [ ] **9.4.divbug.90** Extension / SQL function / VFS registration residue (sibling of `9.4.divbug.66`, carved from `9.4.4.g-unbucketed` 2026-05-16) — **8 pas-soft tests**: 6 `no such extension` (`btree02`, `extension01`, `func4`, `indexexpr2`, …), 1 `no such function` (`func9`), 1 `no such vfs: devsym` (`io`).  Each pin in the C build is a known extension/function/VFS shim; port or auto-register at db-open following the `.66` template.  Progress 2026-05-17: 7/8 registration shims landed (5 aExtension[] rows in TestModuleTest1.pas — eval/fileio/totype/explain/wholenumber, all Pas ports already existed; unistr_quote builtin in passqlite3codegen.pas via quoteFunc + pUserData=1, mirrors func.c:3340).  3/8 now PASS via driver (`extension01`, `func4`, `func9`); 4/8 reach the engine post-shim and re-bucket as non-registration engine bugs (`btree02`, `indexexpr2`, `memdb`, `misc8`); 1/8 (`io`) still wants the full `test_devsym.c` VFS port — out of scope for the registration drain.
  - [ ] **9.4.divbug.90.001** `btree02` — registration shim landed (eval); test now reaches engine.  Residual: btree02-110 expected `[10]` got `[]` — engine divergence, NOT registration.  Triage 2026-05-18: btree02-110 stress-tests the saveCursorPosition/restoreCursorPosition + CURSOR_SKIPNEXT path (see header comment: "making multiple calls to saveCursorPosition() and restoreCursorPosition() when cursors have eState==CURSOR_SKIPNEXT").  The test runs a `SELECT a,ax,b,cnt FROM t1 CROSS JOIN t3 WHERE b IS NOT NULL` cursor and inside the per-row body issues `INSERT INTO t1(a,ax,b)` / `DELETE FROM t1` against the SAME t1, then `COMMIT;BEGIN`.  Each modification must save the outer SELECT's btree cursors (t1 + t1a index) with CURSOR_SKIPNEXT state, then restoreCursorPosition on the next outer Next must re-seek and advance correctly so the final t1 count stays at 10 (5 inserts + 5 deletes).  Pas got `[]` — the iterator runs zero iterations (or all rows deleted), indicating either the saveAllCursors arm or the SKIPNEXT eState restore-then-advance arm is missing/buggy in Pas's btree.c port.  C cite: `src/btree.c` saveCursorPosition (line ~786), restoreCursorPosition (~864), btreeRestoreCursorPosition + the eState==CURSOR_SKIPNEXT branch in btreeNext/btreePrevious (~9000..9100).  Next-agent action: trace `sqlite3BtreeSavePosition` + `btreeRestoreCursorPosition` in src/passqlite3btree*.pas, verify the SKIPNEXT branch decrements/short-circuits the next Move correctly.  Risky deep btree port — deferred from this drain.
  ...
  - [ ] **9.4.divbug.90.005** `indexexpr2` — explain shim landed; test now reaches engine (126→1261).  Residual: engine divergences, NOT registration.  Triage 2026-05-18: 16 distinct subtest failures across at least three independent shapes — (a) `4.200/4.210/4.220` extra `t2` row in the `explain('UPDATE ...')` opcode walk — Pas UPDATE codegen emits an additional `OpenRead` of the main table during its always-ephemeral-rowset two-pass pre-pass, where the C oracle uses a single `OpenWrite` because `sqlite3WhereOkOnePass` returns ONEPASS_MULTI (planner bug in `sqlite3Update` at codegen.pas:38023 — eOnePass detection returning OFF when MULTI is correct); (b) `8.3.1.1..8.3.12.1` (12 subtests) — `SELECT * FROM t0 WHERE ~(' ' BETWEEN t0.c0 AND TRUE)` returns no rows for variants 1..12 of unary-op + BETWEEN over a partial index — partial-index `WHERE c0 NOT NULL` rejects the NULL row but the WHERE expression should still admit it; likely a partial-index where-clause matching bug; (c) `9.0` and `10.1` — generated-column / indexed-expression resolver bugs (similar shape to the 9.4.divbug.88.x indexed-expression family).  Each is a separate bucket; (a) is the most tractable (one OnePass detection bug) but requires the full where-planner audit to fix safely.  Next-agent action: minimise (a) to a single `EXPLAIN UPDATE`, diff Pas's `sqlite3WhereOkOnePass`/where-codegen against `where.c:7218..7245` (the ONEPASS_MULTI emit) and the call-site at `update.c:735..755`.  Deferred from this drain — too risky to land all 3 buckets here.
