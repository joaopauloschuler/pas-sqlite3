...
- [ ] **5.7.b** `sqlite3VdbeSorter*` PMA disk-spill — STUB_INVENTORY #13 DRIFTED-XL (~2400 lines C, vdbesort.c).  In-memory mergesort is real and 1:1; PMA (Packed Memory Array) on-disk merge for sorts larger than the in-memory buffer is not yet ported.  Symptom when missed: large ORDER BY / GROUP BY / CREATE INDEX over multi-page data will fail or silently degrade once the sort buffer overflows.  Complexity: XL.  Blockers: none (self-contained — driver and PMA writer/reader port without touching the VDBE dispatch).
  ...
  - [ ] **5.7.b.1** Structs (C-faithful layout) — declare `TSorterRecord` as C's exact layout: `nVal:i32 @0`, union `pNext:PSorterRecord | iNext:i32 @8` (8-byte alignment → 4 pad bytes at @4), record data at `SRVAL=p+16`; **replaces** the current pNext@0/nVal@8 header.  Add `TMergeEngine` (nTree/pTask/aTree/aReadr), `TPmaReader`, `TPmaWriter`, single-threaded `TIncrMerger` (pTask/pMerger/iStartOff/mxSz/bEof + `aFile[2]`), extend `TSortSubtask` (nPMA/pUnpacked/list/xCompare/file/file2/nSpill) + `TVdbeSorter.aTask[1]`.  Both list-link modes are mandatory: separate-alloc via `u.pNext`, bulk `aMemory` via `u.iNext` offset.  (vdbesort.c:173..461)
  - [ ] **5.7.b.2** Temp-file plumbing — `vdbeSorterOpenTempFile`, `vdbeSorterExtendFile`, `vdbeSorterMapFile` (mmap may stub to `*pp:=nil`→forces buffered reads).  (vdbesort.c:619..634, 1308..1352)
  - [ ] **5.7.b.3** `PmaWriter` — `vdbePmaWriterInit` / `vdbePmaWriteBlob` / `vdbePmaWriteVarint` / `vdbePmaWriterFinish` (buffered page-aligned writes).  (vdbesort.c:1479..1576)
  - [ ] **5.7.b.4** `PmaReader` read primitives — `vdbePmaReaderClear`, `vdbePmaReadBlob`, `vdbePmaReadVarint`, `vdbePmaReaderSeek`, `vdbePmaReaderNext`, `vdbePmaReaderInit`.  (vdbesort.c:474..761)
  - [ ] **5.7.b.5** In-memory rebase + sort-to-PMA + flush + spill trigger — port C's real in-memory engine 1:1, **deleting** the bespoke `vdbeSorterMergeSort`/`vdbeSorterListToArray`/`vdbeSorterCompareRec`: `vdbeSortAllocUnpacked`, `vdbeSorterGetCompare` + `vdbeSorterCompare`/`CompareText`/`CompareInt`/`CompareTail` (operate on `SRVAL`/nVal, with `typeMask` fast paths), `vdbeSorterMerge` (stable two-list merge), `vdbeSorterSort` (bottom-up `aSlot[64]`, iNext→pNext conversion), `vdbeSorterListToPMA`, single-threaded `vdbeSorterFlushPMA`.  Rewrite `sqlite3VdbeSorterInit` (set `mnPmaSize`/`mxPmaSize`/`pgsz` per cache-size + page-size) and `sqlite3VdbeSorterWrite` to C's exact body (`aMemory` bulk-alloc + `iMemory`/`nMemory` realloc + `typeMask` + `bFlush` trigger).  Also rebase `Rewind`/`Next`/`Rowkey`/`Compare` reads onto nVal@0/`SRVAL` so the in-memory (no-spill) path stays green.  Checkpoint: large sorts now spill, but Rewind reads only the final PMA — full read-back lands in 5.7.b.9.  (vdbesort.c:763..934, 936..1048, 1354..1477, 1578..1633, 1727..1796, 1797..1902)
  - [ ] **5.7.b.6** `MergeEngine` — `vdbeMergeEngineNew`/`Free`/`Compare`/`Step`/`Init`/`Level0` (the aTree[] tournament merge).  (vdbesort.c:1193..1229, 1642..1712, 2062..2114, 2144..2218, 2338..2375)
  - [ ] **5.7.b.7** Single-threaded `IncrMerger` — `vdbeIncrMergerNew`/`Free`/`Populate`/`Swap` (bUseThread=0 arms only) + `vdbePmaReaderIncrMergeInit`/`IncrInit` (INCRINIT_NORMAL only).  (vdbesort.c:1909..2047, 2124..2336)
  - [ ] **5.7.b.8** Tree build + merge setup — `vdbeSorterTreeDepth`, `vdbeSorterAddToTree`, `vdbeSorterMergeTreeBuild`, `vdbeSorterSetupMerge` (chooses pReader vs pMerger; `>SORTER_MAX_MERGE_COUNT` hierarchy).  (vdbesort.c:2377..2611)
  - [ ] **5.7.b.9** Public-API PMA arms — `SorterRewind` flushes residual list then sets up merge; `SorterNext`/`SorterRowkey`/`SorterCompare` drive `pReader`/`pMerger` when `bUsePMA`, else fall back to existing in-memory path; `SorterReset`/`Close` free files/mergers/readers.  (vdbesort.c:1247..1306, 1063..1085, 2612..2761)
  - [ ] **5.7.b.10** Verify — force a spill via small `mxPmaSize`/cache_size on a CREATE INDEX / ORDER BY over multi-page data; differential vs C oracle (results + rc).  Confirm the relevant upstream Tcl coverage (`sort.test`, `sort2`/`sort3`, `tkt-*` large-sort cases) flips from soft/skip → strict; add a `Diag` probe only if a narrow divergence needs pinning.
