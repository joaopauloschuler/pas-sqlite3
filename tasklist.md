...
- [ ] **9.4.divbug.14** SQL errors drop object name — routed scattered `sqlite3ErrorMsg` sites through upstream `%s`/`%S`/`%T` formats (build.c / resolve.c).  Residual 2026-05-16: 2 pas-soft test(s) still fail (e.g. aggerror, aggorderby); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.16** `affinity3.test` segfault — `sqlite3WhereBegin` skipped opening a RIGHT JOIN table cursor scanned index-only; ported the `JT_LTORJ|JT_RIGHT` gate (where.c:7252).  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. affinity3); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.19** Table-qualified `rowid` alias (`t1.rowid`, `sp.rowid`) — ported the qualified-case rowid arm from lookupName (resolve.c:471..503 + 623..638) into the TK_DOT branch of `ResolveExpr`: when `sqlite3ColumnIndex` misses and `zCol` ∈ {rowid,oid,_rowid_} and the matched source `HasRowid`, bind `iColumn=-1` / AFF_INTEGER.  Cites: 3fd04ef.  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. autoindex5); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.21** Cross-connection EXCLUSIVE lock detection + busy-handler firing — fixed.  Commits `45593de`, `a8e63c3`.  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. busy); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.24** `sqlite_sequence` double-created for AUTOINCREMENT — ported build.c:2967..2972 (pin `pSchema^.pSeqTab` when init.busy adds a table named `sqlite_sequence`) at codegen.pas:40916.  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. aggnested); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.30** ORDER BY with non-default collation (NOCASE) mis-orders.  Residual 2026-05-16: 3 pas-soft test(s) still fail (e.g. collate4, collate8); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.32** Readonly-DB DELETE returns `unknown error` instead of `attempt to write a readonly database` (delete-8.x).  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. delete); reopened per failing-pas-soft-with-closed-cite rule.
- [ ] **9.4.divbug.33** `count(DISTINCT …)` returns 1 instead of 0 for an empty set (distinctagg-3.x).  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. distinctagg); reopened per failing-pas-soft-with-closed-cite rule.
- [ ] **9.4.divbug.34** `PRAGMA page_size` reports build default (8192) regardless of per-test write (createtab-0.2 expects 4096, format4-1.1 expects 2048).  Residual 2026-05-16: 2 pas-soft test(s) still fail (e.g. createtab, format4); reopened per failing-pas-soft-with-closed-cite rule.
...
- [ ] **9.4.divbug.36** `PRAGMA journal_mode=off` silently ignored — keeps prior mode `delete` (changes-1.1.0).  Residual 2026-05-16: 1 pas-soft test(s) still fail (e.g. changes); reopened per failing-pas-soft-with-closed-cite rule.
