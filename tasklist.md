...
- [~] **9.4.divbug.89** Empty driver diagnostic (carved from `9.4.4.g-unbucketed` 2026-05-16) — **12 pas-soft tests** (`corruptB`, `e_changes`, `e_totalchanges`, `fuzz`, `index4`, `index5`, `join6`, `joinA`, `joinB`, `joinD`, `manydb`, `tkt3080`) FAIL but `bin/tcl-failure-logs/<base>.{err,out}` capture no diagnostic — driver swallows the message or the tests abort outside `tcltest`.  Action: instrument `TclTestDriver` to dump the last N lines of stdout/stderr on any non-PASS exit so these become triageable.  Instrumentation landed (TclTestDriver.pas: `WriteFailLogs` now emits a header — test path, spawn cmd, exit-code, byte counts — and appends a 50-line tail block; empty streams write `(empty)`).  Smoke: join6 now reveals `exit-code: 134` + `double free or corruption (!prev)`; e_changes/manydb reveal `exit-code: 139` (SIGSEGV) past the last `Ok` line.  Per-test root-causing of .001..012 remains TODO.
  ...
  - [ ] **9.4.divbug.89.002** `e_changes` — SIGSEGV (exit-code 139) at `e_changes-4.3.2` (`UPDATE v1 SET y='xyz' WHERE x=1` on `CREATE VIEW v1 AS SELECT * FROM t1` + INSTEAD OF UPDATE trigger).  Triage 2026-05-17: AV at `OP_NewRowid` (passqlite3vdbe.pas:9143) — `apCsr[0]` is nil because Pas codegen never emits `OpenEphemeral 0 2` for the materialised-view ephemeral cursor.  C reference: `select.c:8223..8237` (tag-select-0630) emits `OP_OpenEphemeral pDest->iSDParm, pEList->nExpr` for SRT_EphemTab BEFORE the where loop; Pas has no equivalent — relies on per-arm openers (materialise arm at codegen.pas:32587 opens the *inner* iCsr, not the *outer* iSDParm).  Bug pre-existed divbug.89.004; reproduced under stashed HEAD.  Fix needs new SRT_EphemTab open site in `sqlite3Select` around the SF_NestedFrom check — ~5 lines but must coexist with existing per-arm openers (audit for double-Open).  DEFERRED — separate from 89.004 codegen-arm-gate work.
