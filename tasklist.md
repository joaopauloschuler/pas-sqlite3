...
- [ ] **9.4.port.coroutine-from** Coroutine FROM-subquery materialization. **2026-06-06 re-diagnosis (memory was stale — narrowed the actual blocker):** the viaCoroutine *machinery* is ALREADY ported — `constructAutomaticIndex` viaCoroutine fill (codegen.pas:26087, no longer an assert), WhereBegin consumer InitCoroutine/Yield (27980), wherecode arm (28640), column→copy rewrite (28193), autoindex eligibility gate does NOT exclude viaCoroutine (22408). The remaining blocker is narrower: the **single-source FROM-subquery coroutine consumer is a HAND-ROLLED OP_Yield scan loop at codegen.pas:40878..41016 that NEVER calls `sqlite3WhereBegin`**, so the planner (whereLoopAddAll → whereLoopAddBtree → constructAutomaticIndex) is bypassed entirely for it. Verified via instrumentation (mini repro `SELECT a,(SELECT v.c FROM v WHERE v.d=t1.b ORDER BY v.c) FROM t1` over a DISTINCT+ORDER-BY view v): oracle emits `CO-ROUTINE v` + `BLOOM FILTER ON v (d=?)` + `SEARCH v USING AUTOMATIC COVERING INDEX (d=?)`; Pas emits `CO-ROUTINE v` + `SCAN v` (no addBtree call for v's cursor at all). The multi-source/materialise path (40878 else-arm → 39000-ish) DOES go through WhereBegin and DOES build the auto-index correctly (mini4 `SELECT * FROM t1,v WHERE v.d=t1.b` matches oracle's SEARCH..AUTOMATIC COVERING INDEX). **Fix = route the single-source coroutine consumer through sqlite3WhereBegin like C (select.c codes the FROM coroutine but the consumer scan ALWAYS goes through WhereBegin).** NOT landed: high regression risk — the 40878 hand-roll is load-bearing for hundreds of single-source FROM-subquery tests (10.1.bug.40/.95/.113, divbug.89.004, values-15.x, VALUES coroutine), and the only EQP guard in-tree (`bin/TestExplainParity`) is a bytecode-diff scaffold (224 pass/802 diverge on inserted OP_Explain ops — NOT the "1026/1026" the older memory cites), so a coroutine-path rewrite cannot be reliably regression-gated short of the full Tcl sweep. A broad "outer has WHERE → materialise" fall-through loses the CO-ROUTINE EQP node for every such subquery → likely many regressions; a precise gate amounts to re-deriving fromClauseTermCanBeCoroutine + cost. Deferred to a dedicated session with the full Tcl sweep as the gate. **ROOT of 4 live FAILs**: divbug.19 (autoindex5-1.1), divbug.92.009 (with5-310), pushdown-6.x, update-21.12. Sub-fail autoindex5-3.0 (windowed dup row_number) is a separate gap: `gatherSelectWindows` (expr.c:1987) is still UNported (now a clean "misuse of aggregate function row_number()" error, no longer a SIGSEGV) and must land together with the addrFillSub single-coding dedup or it converts the error to a hang.
