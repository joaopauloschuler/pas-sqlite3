...
- [ ] **9.4.divbug.28** EXPLAIN QUERY PLAN segfaults on multi-table queries after first row (eqp2/cost/fordelete/delete2).  Cite: vdbe.c `OP_Explain` ~7100, vdbeaux.c `sqlite3VdbeExplain`.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.29** TEXT-affinity column stores `'0x119'` literal as INTEGER 281 (collate1).  Cite: vdbe.c `applyAffinity` TEXT arm.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.30** ORDER BY with non-default collation (NOCASE / index-uses-collation) mis-orders or picks `sort` instead of `nosort` (collate2/8/9, collate4).  Cite: callback.c `sqlite3FindCollSeq`, where.c `wherePathSatisfiesOrderBy`.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.31** Spurious `database disk image is malformed` for non-corrupt errors (collate3 `no such collation sequence: …`, count-1.2.4/5).  Cite: callback.c `sqlite3LocateCollSeq`, main.c `sqlite3ErrStr`.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.32** Readonly-DB DELETE returns `unknown error` instead of `attempt to write a readonly database` (delete-8.x).  Cite: main.c `sqlite3ErrStr`, btreeBeginTrans RO arm.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.33** `count(DISTINCT …)` returns 1 instead of 0 for an empty set (distinctagg-3.x).  Cite: func.c `countStep` DISTINCT path, select.c `updateAccumulator` DISTINCT loop.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.34** `PRAGMA page_size` reports build default (8192) regardless of per-test write (createtab-0.2 expects 4096, format4-1.1 expects 2048).  Cite: pragma.c PragTyp_PAGE_SIZE write arm, pager.c `sqlite3PagerSetPagesize`.  Surfaced 9.4.4.e.
- [ ] **9.4.divbug.35** Float-to-text precision artefacts: `-1.11` → `-1.1099999999999999`; large doubles get an extra mantissa digit (fpconv1, default-3.3).  Cite: printf.c `etFLOAT`, util.c `sqlite3_str_appendf` `%g` arm.  Surfaced 9.4.4.e.
