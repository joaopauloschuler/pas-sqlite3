...
- [X] **10.1.48** `.recover` — extension dispatcher landed (passqlite3recover.pas, ~957 lines + LAF arm + wrapper-VFS arm).  Three porting bugs closed 2026-05-11:
  ...
  - [ ] **10.1.48.d** (post-10.1.48 follow-up) Land the `recoverGetPage` page-1 swap-back (mirrors `sqlite3recover.c:723`) so `.recover` re-emits the original schema + data, then fix the `free(): invalid pointer` surfaced by `recoverWriteData` after the first emitted INSERT.  Once both land, tighten `DiagRecover` to a byte-parity diff against `/home/bpsa/app/sqlite3/sqlite3 .recover` on the same fixture.
