...
- [~] **10.1.48** `.recover` — extension dispatcher landed (passqlite3recover.pas, ~957 lines + LAF arm + wrapper-VFS arm).  Three porting bugs closed 2026-05-11:
  ...
  - [~] **10.1.48.b** `WHERE pgno IN (SELECT p FROM pages)` over a recursive CTE: root-caused, not closed.  `sqlite3CodeRhsOfIN`'s Case-1 calls `sqlite3SelectDup` then `sqlite3Select` on the duplicate; the duplicate's pSrc[0]="pages" still has isCte+pSubq pointing at the recursive UNION ALL, but the inner `sqlite3Select` never re-fires `generateWithRecursiveQuery` (gated on `SF_Recursive + hasAnchor` of the *outer* compound at `codegen.pas:25106`, not on a SrcItem-attached recursive subquery).  Bytecode shows OP_Rewind on an un-opened iRecTab cursor — execution AV at `vdbe.pas:7910 OP_Rewind`.  Independent of 10.1.48.a: reproduces with plain `SELECT p FROM t WHERE p IN (SELECT p FROM pages_recursive)` (SRT_Output destination).  Added `pCteUse->nUse` bump in `sqlite3SrcListDup` (mirrors C `expr.c sqlite3SrcListDup`) — correct per upstream but not sufficient.  Real fix needs the IN-RHS sqlite3Select to materialise the recursive CTE eph table before iterating it (probably by routing the SrcItem-attached pSubq^.pSelect through generateWithRecursiveQuery during the IN-RHS compile).  New diag: `src/tests/DiagInRecursive.pas`, `src/tests/DiagExplainRec.pas`.
  - [ ] **10.1.48.c** Land a `DiagRecover` / `TestRecover` byte-parity gate under `src/tests/`.  Blocked on 10.1.48.b — `recoverCacheSchema` cannot run end-to-end until the recursive `pages` CTE materialises in the IN-RHS.  Once unblocked: run `.recover` against the corruption fixtures from `../sqlite3/ext/recover/recover_common.tcl` (or a minimal locally-generated corrupt db) and diff the SQL stream against `/home/bpsa/app/sqlite3/sqlite3 .recover` on the same input.  Wire into `build.sh` and the regression so 10.1.48 has a real gate.  Required to flip the bullet from `[~]` to `[X]`.
- [X] **10.1.49** `.dbinfo`.
