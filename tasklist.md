...
- [ ] **9.4.divbug.94** 240 s watchdog timeouts, 0 subtests, formerly cited to closed buckets — sort4 (ex-63), e_fkey (ex-38), e_walauto (ex-87), fuzz (ex-89).  Timeout class: root-cause per-test slowness or raise the per-test budget knob before re-tagging.
  - [X] **9.4.divbug.94.e_walauto** CLOSED 2026-06-10: PASS 42/42 (~0.5 s warm).  Three stacked causes.  (1) Missing testfixture command: `sqlite3_wal_autocheckpoint` (test1.c:7745..7776, registered at test1.c:9289) was never ported — added to TestModuleTest1.pas (all 1.2.x "invalid command name" errors).  (2) ENGINE: the PragTyp_WAL_AUTOCHECKPOINT arm in codegen re-derived zRight from the raw pValue token, dropping the minusFlag sign restored at the top of sqlite3Pragma (pragma.c:463..467), so `PRAGMA wal_autocheckpoint = -4` parsed as +4 and installed a 4-frame default hook instead of disabling autocheckpoint (e_walauto-1.1.9, R-26993-43540) — now uses the function-level zRight, matching pragma.c:2421..2429.  (3) HARNESS/BUILD: the upstream testfixture links with `-DSQLITE_NO_SYNC=1` (main.mk:1797) so full_fsync is an fstat-only no-op (os_unix.c:3810..3814), but our Tcl test .so did real fsync()s (~19 ms each on this ext4 /tmp) — the test's 4×10000-insert WAL loops then blew the 240 s watchdog (timed: 355 s; oracle testfixture does the same loops in ~2 s).  Added pas_full_fsync to passqlite3os.pas ({$ifdef SQLITE_NO_SYNC} → FpFStat validate, else real fsync; SQLITE_TEST sync counters still bump at the call sites as in C) and -dSQLITE_NO_SYNC to build_tcl_lib.sh only — build.sh/production unaffected.  Collateral: e_fkey PASS 940/19 s (same fsync slowness — promoted to pas-strict); e_wal/e_walhook/io/sync/sync2/wal64k/wal7/8/9/walbig/walhook/walnoshm/waloverwrite/walshared/backup/crash8/ioerr/journal1/jrnlmode/select1/insert/update/tempdb all PASS on the NO_SYNC build.
  - [X] **9.4.divbug.94.e_fkey** CLOSED 2026-06-10 with the NO_SYNC fix above: PASS 940 subtests / ~19 s; promoted to pas-strict.
  - [X] **9.4.divbug.94.sort4** CLOSED 2026-06-10: PASS 10 subtests / ~204 s.  NOT a slowness bug — sort4 died at `SOURCE-ERROR: missing operand at _@_` in `expr 0-_@_`: its `clock_seconds` proc is `db one {SELECT strftime('%s')}` (single-arg), and the Pas datetime family diverged from date.c isDate: (a) strftimeFunc returned NULL for `argc < 2`, but C (date.c:1422..1424) calls `isDate(context, argc-1, argv+1)` so argc==1 hits isDate's argc==0 current-time path (date.c:1117..1119) — clock_seconds returned empty and Tcl `expr` blew up; (b) date/time/datetime/julianday/unixepoch treated a NULL first arg as "read current time", but C only reads current time for argc==0 — a NULL arg flows to `z = sqlite3_value_text(argv[0]) == 0 → return 1 →` NULL result (date.c:1121..1128).  Fixed all six funcs to mirror isDate; collateral: date.test (tcl-internal) 66→53 errors — date-7.1/.4/.7/.10/.14 now pass, zero new failures (remaining date-6/11/19 are pre-existing LOCALTIME_FAULT + ceiling/floor gaps).  sort4 legitimately needs ~204 s → added to PER_TEST_TIMEOUT_OVERRIDES (300 s).  date2/date3/date4 PASS.
...
- [X] **9.4.divbug.99** soak.test deferral — CLOSED 2026-06-10: PASS 175 subtests / 34.8 s, promoted to pas-strict.  Harness gap, not an engine divergence.  Fixes: (a) ported the test_thread.c clock commands — `clock_seconds` (thread-interp spelling, test_thread.c:119) plus `clock_second`/`clock_milliseconds` (main-interp spellings, test_thread.c:643..644) — into Sqlite3_Init (PasTclSqlite.pas, Tcl_GetTime-backed like clock_seconds_proc at test_thread.c:368..395; sqlthread itself stays unported); (b) driver grew PER_TEST_ARGV_OVERRIDES — soak.test gets `::argv {-timeout 1}`, mirroring `testfixture test/soak.test -timeout N` (soak.test:27..38), so the run loop (soak.test:75) executes exactly one iteration: fuzz.test in quick mode (`G(isquick)=1` → REPEATS 20, fuzz.test:30..32), which sidesteps (c) fuzz.test's full-REPEATS slowness (divbug.94 still open for standalone fuzz.test).  Per-test watchdog override 300 s (measured 35 s > 30 s default).
- [X] **9.4.divbug.100** walcrash / walcrash3 stale pas-strict — CLOSED 2026-06-10: walcrash PASS 2970 subtests / 517 s (/tmp/run_walcrash.log); walcrash3 PASS 20996 subtests / 1278 s (/tmp/run_walcrash3.log), oracle parity (C testfixture baseline /tmp/owc3/out.log: "0 errors out of 20997 tests", 20996 Ok lines).  Three stacked causes.  (1) HARNESS root cause — the crash VFS (TestModuleCrash.pas) left xShmMap/xShmLock/xShmBarrier/xShmUnmap nil, so in the child `sqlite3PagerWalSupported()` silently refused `PRAGMA journal_mode=WAL`: the crash-armed test.db-wal was never created, cfSync never fired, and the child exited NORMALLY (`expected [1 {child process exited abnormally}] got [0 {}]` + follow-on `expected wal got delete`).  Ported the test6.c:548..592 pass-throughs (dispatch on pReal->pMethods->xShm*, as in C); also env-gated CRASHTRACE debug prints mirroring TRACE_CRASHTEST.  (2) HARNESS — tester_min.tcl `crashsql` wrote `catch { sqlite3_test_control_pending_byte \$::sqlite_pending_byte }` into the child script: deferred `\$` + catch made the child silently keep the default PENDING_BYTE 0x40000000 while the parent uses 0x10000, so child-written pages at the parent's locking page read back as corruption (walcrash-5/6).  Upstream tester.tcl:1794 interpolates the value with no catch — now we do too.  (3) ENGINE — unixLockSharedMemory carried a local "only truncate when -shm <= 3 bytes" guard; os_unix.c:4906 truncates UNCONDITIONALLY, because a leftover -shm from a crashed process holds an internally-valid wal-index header — skipping the truncate skips WAL recovery and trusts stale frame counts.  The guard existed to protect same-process second connections, but those never reach this path in C or Pas: unixOpenSharedMemory reuses pInode^.pShmNode.  Removed the guard.  Per-test watchdog overrides added (walcrash 1200 s, walcrash3 7200 s); both tests promoted back to pas-strict. (discovered 2026-06-10 during divbug.94 e_walauto triage; pre-existing at HEAD f4c05d1, verified with BOTH the old real-fsync build and the new NO_SYNC build — identical failures, so NOT caused by the NO_SYNC change).  Two layers: (a) at HEAD both tests blow the 240 s watchdog (real fsyncs); (b) with NO_SYNC walcrash completes (230 s) but 1683 of 2970 subtests fail — the `crashsql` child process exits NORMALLY instead of crashing (`expected [1 {child process exited abnormally}] got [0 {}]`, e.g. walcrash-1.1.1/1.1.4, and follow-on `expected wal got delete` journal-mode checks).  The crash-VFS cfSync trigger (TestModuleCrash.pas, port of test6.c:469..496) is apparently never reached for the WAL-file sync the test arms (`-file test.db-wal -delay N`) — suspect the Pas WAL/pager sync-path gating or the crash-VFS wiring in the child; root-cause against test6.c + wal.c sync arms.  walcrash/walcrash3 STATUS flipped pas-strict → pas-soft citing this bucket.
