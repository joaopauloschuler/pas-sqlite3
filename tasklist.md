...
- [~] **10.1.48** `.recover` — extension dispatcher landed (passqlite3recover.pas, ~957 lines + LAF arm + wrapper-VFS arm).  Three porting bugs closed 2026-05-11:
  ...
  - [ ] **10.1.48.a** Thread `SRT_Coroutine` (and `SRT_EphemTab`) through the single-source GROUP BY agg arm at `codegen.pas:25468`.  Today the gate is `eDest in {SRT_Output, SRT_Mem}`; the per-row emit at the outputRow Gosub must route through `selectInnerLoop`'s SRT_Coroutine / SRT_EphemTab arms (OP_MakeRecord+Yield for coroutine consumers, OP_MakeRecord+NewRowid+Insert for eph-table consumers) instead of the direct OP_ResultRow.  Mirror `select.c:8569..8625`.  Unblocks `INSERT INTO recovery.schema SELECT … GROUP BY pgno, cell` and the identical shape in both LAF arms (`passqlite3recover.pas:1096` and `:1545`).
  - [ ] **10.1.48.b** Verify `WHERE pgno IN (SELECT p FROM pages)` (correlated CTE membership test) flows through the new GROUP BY/coroutine path.  After 10.1.48.a lands, re-check whether `sqlite3CodeRhsOfIN` materialises the recursive `pages` CTE into an ephemeral set; if it short-circuits when the outer destination is a coroutine, extend the IN-RHS codegen to accept that arm.  May be a no-op if WhereBegin's existing IN-set handling already cooperates.
  - [ ] **10.1.48.c** Land a `DiagRecover` / `TestRecover` byte-parity gate under `src/tests/`.  Run `.recover` against the corruption fixtures from `../sqlite3/ext/recover/recover_common.tcl` (or a minimal locally-generated corrupt db) and diff the SQL stream against `/home/bpsa/app/sqlite3/sqlite3 .recover` on the same input.  Wire into `build.sh` and the 5020-assertion regression so 10.1.48 has a real gate (today the recovery dispatcher has no test coverage).  Required to flip the bullet from `[~]` to `[X]`.
- [X] **10.1.49** `.dbinfo`.
